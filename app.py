...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def validate_vacation_exchange(self, sender, receiver, turn):
        ...
        for period in ('1차', '2차'):
            s_vac = sv[period]
            r_vac = rv[period]
            s_in_turn = bool(s_vac and s_vac.get('turn') == turn)
            r_in_turn = bool(r_vac and r_vac.get('turn') == turn)
...
| 🏖️ 휴가턴 교환 | **휴가턴은 휴가 턴끼리만 교환 가능**해요. 나와 상대방 모두 같은 턴에 휴가가 배정되어 있어야 교환할 수 있어요 (예: 둘 다 5턴이 휴가면 OK) |
