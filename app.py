...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def process_request(self, req_id, action):
        ...
        if req.get('status') != 'pending':
            return False, f"이미 처리된 요청입니다. (상태: {req['status']})"
...
if user == 'ADMIN':
    ...
    with adm_tab1:
        if mgr.df.empty:
            ...
            st.caption("각 턴 전환 구간별로 구미에 연속 잔류하는 인턴 명단입니다.")

            # 턴을 숫자 순서대로 정렬
            turn_cols = sorted(mgr.df.columns, key=lambda c: int(re.sub(r'\D', '', c)) if re.sub(r'\D', '', c) else 0)

            # 각 인턴의 턴별 구미 여부 계산
            intern_gumi = {}  # {intern: {turn: bool}}
            for intern in mgr.df.index:
                intern_gumi[intern] = {}
                for t in turn_cols:
                    val = mgr.df.loc[intern, t]
                    if not val or str(val) in ('None', '', 'nan'):
                        intern_gumi[intern][t] = False
                    else:
                        loc, _ = mgr.parse_cell(val)
                        intern_gumi[intern][t] = (loc or DEFAULT_LOCATION) == '구미'

            # 턴 전환별 연속 잔류 인턴 목록
            transition_rows = []
            for i in range(len(turn_cols) - 1):
                t_from = turn_cols[i]
                t_to   = turn_cols[i + 1]
                staying = []
                for intern in mgr.df.index:
                    if intern_gumi[intern][t_from] and intern_gumi[intern][t_to]:
                        v1 = str(mgr.df.loc[intern, t_from])
                        v2 = str(mgr.df.loc[intern, t_to])
                        staying.append(f"{intern}({v1}→{v2})")
                transition_rows.append({
                    '턴 전환': f"{t_from} → {t_to}",
                    '잔류 인원': len(staying),
                    '잔류 명단': ', '.join(staying) if staying else '-',
                })

            trans_df = pd.DataFrame(transition_rows)
            has_any = any(r['잔류 인원'] > 0 for r in transition_rows)

            if has_any:
                max_stay = max(r['잔류 인원'] for r in transition_rows)
                total_transitions = sum(1 for r in transition_rows if r['잔류 인원'] > 0)
                gc1, gc2 = st.columns(2)
                gc1.metric("연속 잔류 발생 구간 수", f"{total_transitions}개")
                gc2.metric("최대 동시 잔류 인원", f"{max_stay}명")

            def _highlight_gumi(row):
                if row['잔류 인원'] > 0:
                    return ['background-color:#fff3e0'] * len(row)
                return [''] * len(row)

            st.dataframe(
                trans_df.style.apply(_highlight_gumi, axis=1),
                use_container_width=True, hide_index=True,
                height=min(80 + len(trans_df) * 35, 500)
            )

            # 인턴별 구미 연속 구간 요약
            st.markdown("**인턴별 구미 연속 구간 요약**")
            intern_streak_rows = []
            for intern in mgr.df.index:
                current_streak = []
                streaks = []
                for t in turn_cols:
                    if intern_gumi[intern][t]:
                ...
                for streak in streaks:
                    ...
                    intern_streak_rows.append({
                        '인턴': intern,
                        '연속 턴 수': len(streak),
                        '구간': f"{streak[0]}~{streak[-1]}",
                        '배치': ' → '.join(vals),
                    })
            if intern_streak_rows:
                streak_df = pd.DataFrame(intern_streak_rows).sort_values(
...
with st.sidebar:
    ...
    _sim_modes = ["🔄 특정 턴 교환", "🎯 특정 턴 받기", "🔗 복합 교환 탐색"]
    sim_mode = st.radio(
        "모드",
        _sim_modes,
        key="sim_mode_radio",
...
with tab_browse:
    if not others_posts:
        ...
        for post in others_posts:
            ...
            _poster_tag = ""
            _valid_label = f"✅ {valid_cnt}개 조합" if has_valid else "❌ 단독 불가 (복합 탐색 가능)"
            with st.expander(
...
with tab_post:
    ...
    all_turns_p = [c for c in mgr.df.columns if c not in LOCKED_TURNS
                   and (user not in mgr.df.index
                        or ('진로선택' not in str(mgr.df.loc[user, c])
                            and '진로탐색' not in str(mgr.df.loc[user, c])))] if not mgr.df.empty else []
...
- ➕ 버튼으로 **여러 턴을 한꺼번에** 교환 신청할 수 있어요 (복합교환)

> ⛔ **1턴, 2턴**은 교환이 불가능한 턴이에요
> ⛔ **진로선택, 진로탐색**은 교환이 불가능한 과목이에요
...
한 번에 **여러 상대방 × 여러 턴**을 묶어서 교환할 수 있어요.

**사용 방법**
1. 교환 신청 화면에서 `➕ 항목 추가` 버튼을 눌러 교환할 턴을 추가하세요
2. 각 항목마다 상대방과 턴을 선택하세요
3. 모든 조건이 충족되면 **한꺼번에 요청**이 발송돼요

> 💡 복합교환은 **모든 상대방이 수락해야만** 일괄 실행돼요. 한 명이라도 거절하면 전체 취소!
> 💡 휴가턴을 주면서 다른 휴가턴을 받아오는 경우에 유용해요

---

### 🔍 5. 교환 가능한 조합 찾기

어떤 교환이 가능한지 **자동으로 탐색**해주는 기능이에요.

**사이드바 → 교환 시뮬레이션**에서 사용할 수 있어요.

| 모드 | 설명 |
|------|------|
| 🔄 **특정 턴 교환** | 특정 턴을 기준으로 교환 가능한 파트너 전체 목록을 보여줘요 |
| 🎯 **특정 과목 받기** | 내가 받고 싶은 과목(복수 선택 가능!)을 얻을 수 있는 모든 (파트너, 턴) 조합을 탐색해요 |

- 결과에서 `요청` 버튼을 누르면 바로 교환 신청 화면으로 연결돼요
- ⏳ 표시: 이미 대기 중인 요청이 있어요

**메인 화면 → 복합교환 조합 탐색**에서도 사용할 수 있어요.

| 기능 | 설명 |
|------|------|
| 🧩 **조합 탐색** | 내가 얻고 싶은 과목/값을 선택하면, 교환 가능한 **복합교환 조합(체인)**을 자동으로 찾아줘요 |
| 🏪 **장터 조합 탐색** | 장터에 올라온 게시물과 매칭되는 복합교환 조합을 찾아줘요 |

> 💡 조합 탐색은 필수과목·휴가·분당 규칙을 모두 검증한 **유효한 조합만** 보여줘요
...
| 교환 불가 과목 | **진로선택**, **진로탐색**이 배정된 턴은 교환할 수 없어요 |
| 🏖️ 휴가턴 교환 | 휴가턴을 줬으면 **같은 상대방에게서 다른 휴가턴을 받아와야** 해요 (예: 5턴 휴가를 주고, 9턴 휴가를 받기) |
| 복합교환 | 여러 턴을 묶어서 한꺼번에 교환할 수 있어요. 모든 상대방이 수락해야 실행돼요 |
| 교환 횟수 | **제한 없음** — 규칙만 충족하면 원하는 만큼 교환할 수 있어요! |
