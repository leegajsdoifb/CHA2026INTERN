...
VACATION_HOLIDAY_SHEET  = '2026년 입사 인턴 스케쥴_휴가'
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def connect_google_sheet(self):
        try:
            ...
            # 휴가 스케줄 탭 (최종본)
            try:
                self.vac_holiday_ws = self.sh.worksheet(VACATION_HOLIDAY_SHEET)
            except gspread.WorksheetNotFound:
                self.vac_holiday_ws = None
    ...
    def _parse_vacation_sheet(self, ws):
        ...
        try:
            ...
            turn_cols = [(i, h.strip()) for i, h in enumerate(header) if re.search(r'\d+턴', h)]

            # 데이터 시작 행 (헤더 바로 다음, 날짜 행이 있으면 건너뜀)
    ...
    def simulate_multi_swap(self, name, only_need_multi=True, max_swaps=3,
        ...
        def check_combo(combo_indices):
            ...
            # 휴가 수지 검증 (상대방별 균형)
            combo_exchanges = [{'target': s['partner'], 'turn': s['turn']} for s in swaps]
            ok_vb, _ = self.validate_vacation_balance(name, combo_exchanges)
            if not ok_vb:
                return None
    ...
    def add_chain_request(self, sender, swaps, message='', messages=None):
        ...
        # 휴가 수지 검증 (상대방별 균형)
        chain_exchanges = [{'target': r['receiver'], 'turn': r['turn']} for r in created_reqs]
        ok_vb, vb_errs = self.validate_vacation_balance(sender, chain_exchanges)
        if not ok_vb:
            return False, "\n".join(vb_errs)
    ...
    def find_completing_combos(self, sender, mandatory_exchanges, max_additional=2):
        ...
        def check_combo(additional_list):
            ...
            for s in all_swaps:
                pa, pb, t = (s.get('comp_owner', sender) if s.get('is_partner_comp') else sender), s['partner'], s['turn']
            ...
            # 휴가 수지 검증 (나의 교환만 — 설정 시 실제 적용되는 범위)
            combo_ex = [{'target': s['partner'], 'turn': s['turn']} for s in sender_swaps]
            ok_vb, _ = self.validate_vacation_balance(sender, combo_ex)
            if not ok_vb:
                return None
    ...
    def process_chain_action(self, chain_id, req_id, action, actor):
        ...
        if action == 'accept':
            ...
            if self.admin_settings.get('exchange_limit_enabled'):
                ...
                if self.count_accepted_exchanges(sender) >= limit:
                    over.append(sender)
                all_receivers = set(r['receiver'] for r in chain_reqs)
            ...
            # 휴가 수지 검증 (상대방별 균형)
            chain_exchanges = [{'target': r['receiver'], 'turn': r['turn']} for r in chain_reqs]
            ok_vb, vb_errs = self.validate_vacation_balance(sender, chain_exchanges)
            if not ok_vb:
                all_valid = False
                fail_msgs.extend(vb_errs)
    ...
    def count_accepted_exchanges(self, name):
        """해당 인턴의 수락된 교환 횟수를 반환.
        - 단건 교환: 각각 1회
        - 체인(복합) 교환: 같은 chain_id는 1회로 카운트
        """
        counted_chains = set()
        count = 0
        for req in self.requests:
            ...
            if chain_id:
                if chain_id not in counted_chains:
                    counted_chains.add(chain_id)
                    count += 1
            else:
                count += 1
        return count
    ...
    def _has_vacation_on_turn(self, name, turn):
        """해당 인턴이 특정 턴에 휴가가 있는지 여부"""
        vac = self.vacation_data.get(name, {})
        for period in ('1차', '2차'):
            v = vac.get(period)
            if v and v.get('turn') == turn:
                return True
        return False
    ...
    def validate_vacation_exchange(self, sender, receiver, turn):
        ...
        """
        errors = []
        s_has = self._has_vacation_on_turn(sender, turn)
        r_has = self._has_vacation_on_turn(receiver, turn)
    ...
    def validate_vacation_balance(self, sender, exchanges):
        """
        복합 교환에서 상대방별 휴가 턴 수지 검증.
        규칙: 상대방에게 휴가턴을 줬으면, 같은 상대방에게서 휴가턴을 받아와야 함.
        반환: (ok: bool, errors: list)
        """
        errors = []
        # 상대방별 휴가 수지
        pair_balance = {}   # {target: {'given': n, 'received': n}}

        for ex in exchanges:
            target = ex['target']
            turn   = ex['turn']
            if target not in pair_balance:
                pair_balance[target] = {'given': 0, 'received': 0}
            if self._has_vacation_on_turn(sender, turn):
                pair_balance[target]['given'] += 1
            if self._has_vacation_on_turn(target, turn):
                pair_balance[target]['received'] += 1

        for target, bal in pair_balance.items():
            if bal['given'] != bal['received']:
                if bal['given'] > bal['received']:
    ...
    def validate_multi_exchange(self, sender, exchanges):
        ...
        # ── 휴가 수지 검증 (상대방별 휴가턴 주고받기 균형) ────────────────────
        ok_vb, vb_errs = self.validate_vacation_balance(sender, exchanges)
        if not ok_vb:
...
# ── 교환 제한 전역 변수 ──
_exchange_limit_active = mgr.admin_settings.get('exchange_limit_enabled', False)
_exchange_limit_count = mgr.admin_settings.get('exchange_limit_count', 1)
    ...
    with adm_tab2:
        if mgr.df.empty:
            ...
            if sel_intern:
                ...
                _sel_vac_info = mgr.get_intern_vacation(sel_intern)
                # 휴가 턴 → 타입 매핑
                _sel_vac_type_map = {}
                for _vp in ('1차', '2차'):
                    _vi = _sel_vac_info.get(_vp)
                    if _vi and _vi.get('turn'):
                        _sel_vac_type_map[_vi['turn']] = _vi.get('type', '')
                detail_rows = []
                for col in mgr.df.columns:
                    val = row[col]
                    ...
                    detail_rows.append({
                        '턴': col,
                        '휴가': _sel_vt.get(col, ''),
                        '휴가타입': _sel_vac_type_map.get(col, ''),
                        '배치값': val or '-',
                        '과목': dept or '-',
                ...
                for _vp in ('1차', '2차'):
                    _vi = _sel_vac_info.get(_vp)
                    if _vi and _vi.get('turn'):
                        _adm_vac_parts.append(f"**{_vp}** {_vi['turn']} ({_vi.get('type', '-')})")
                if _adm_vac_parts:
                    st.caption('🏖️ 휴가: ' + ' · '.join(_adm_vac_parts))
...
with st.sidebar:
    ...
    # 휴가턴 정보 표시
    _user_vac = mgr.get_intern_vacation(user)
    _vac_parts = []
    for _vp in ('1차', '2차'):
        _vi = _user_vac.get(_vp)
        if _vi and _vi.get('turn'):
            _vac_parts.append(f"**{_vp}** {_vi['turn']} ({_vi.get('type', '-')})")
    if _vac_parts:
        st.caption('🏖️ 휴가: ' + ' · '.join(_vac_parts))
    else:
        st.caption('🏖️ 휴가: 배정 없음')

    # 교환 기회 표시
    if mgr.admin_settings.get('exchange_limit_enabled'):
        _sb_limit = mgr.admin_settings.get('exchange_limit_count', 1)
        _sb_used = mgr.count_accepted_exchanges(user)
        _sb_remain = max(0, _sb_limit - _sb_used)
        if _sb_remain > 0:
            st.caption(f"🔄 교환: **{_sb_remain}회 남음** ({_sb_used}/{_sb_limit})")
        else:
            st.caption(f"⛔ 교환: **기회 소진** ({_sb_used}/{_sb_limit})")
            ...
            if not inbox_sb:
                ...
                for req in inbox_normal:
                    ...
                    if _exchange_limit_active:
                        _snd_over = mgr.count_accepted_exchanges(req['sender']) >= _exchange_limit_count
                        _rcv_over = mgr.count_accepted_exchanges(user) >= _exchange_limit_count
                        if _snd_over or _rcv_over:
                            _who = []
                            if _snd_over: _who.append(f"{req['sender']}(신청자)")
                            if _rcv_over: _who.append("나")
                            st.warning(f"⛔ {', '.join(_who)}의 교환 기회가 소진되어 수락 불가")
                            _req_blocked = True
                    ca, cb = st.columns(2)
                    if ca.button("✅ 수락", key=f"sb_acc_{req['id']}", type="primary",
                                 use_container_width=True, disabled=_req_blocked):
                ...
                for cid, reqs in chain_groups.items():
                    ...
                    _chain_blocked = False
                    if _exchange_limit_active:
                        _chain_over = []
                        _chain_sender = reqs[0]['sender']
                        if mgr.count_accepted_exchanges(_chain_sender) >= _exchange_limit_count:
                            _chain_over.append(f"{_chain_sender}(신청자)")
                        if mgr.count_accepted_exchanges(user) >= _exchange_limit_count:
                            _chain_over.append("나")
                        if _chain_over:
                            st.warning(f"⛔ {', '.join(_chain_over)}의 교환 기회가 소진되어 수락 불가")
                            _chain_blocked = True
                    ca, cb = st.columns(2)
                    # 체인은 내게 해당하는 건만 일괄 수락/거절
                    my_chain_reqs = [r for r in reqs if r['status'] == 'pending']
                    if ca.button("✅ 수락", key=f"sb_chacc_{cid}", type="primary",
                                 use_container_width=True, disabled=_chain_blocked):
                        last_msg = ""
    ...
    _sim_modes = ["🔄 특정 턴 교환", "🎯 특정 턴 받기"]
    if not _exchange_limit_active or _exchange_limit_count > 1:
        _sim_modes.append("🔗 복합 교환 탐색")
    sim_mode = st.radio(
        "모드",
        _sim_modes,
        key="sim_mode_radio",
...
# ── 교환 횟수 제한 상태 표시 ──
_my_exchange_count = mgr.count_accepted_exchanges(user) if user else 0
_exchange_remaining = max(0, _exchange_limit_count - _my_exchange_count) if _exchange_limit_active else None

if _exchange_limit_active:
    if _exchange_remaining > 0:
...
for i, item in enumerate(st.session_state.exchange_items):
    ...
    with c1:
        ...
        if _exchange_limit_active:
            _others_labels = []
            for _o in others:
                _o_rem = _exchange_limit_count - mgr.count_accepted_exchanges(_o)
                _o_tag = f"⛔" if _o_rem <= 0 else f"🔄{_o_rem}"
                _others_labels.append(f"{_o}  {_o_tag}")
        else:
            _others_labels = others
        _ei_t_kwargs = {"key": _ei_t_key,
                        "format_func": lambda idx, _lb=_others_labels: _lb[idx]}
        if _ei_t_key not in st.session_state:
            _ei_t_kwargs["index"] = others.index(t_default)
        _sel_t_idx = st.selectbox(lbl, range(len(others)), **_ei_t_kwargs)
        sel_t = others[_sel_t_idx]
    with c2:
        # 턴 선택 시 휴가 표시
    ...
    if _exchange_limit_active:
        _sel_t_rem = _exchange_limit_count - mgr.count_accepted_exchanges(sel_t)
        if _sel_t_rem <= 0:
            st.warning(f"⛔ **{sel_t}**의 교환 기회가 소진되어 교환 신청이 불가합니다.")
...
if st.session_state.exchange_items:
    ...
    if can_send:
        ...
        _hide_multi = _exchange_limit_active and _exchange_limit_count <= 1
        _do_find_combo = False
        if not _hide_multi:
            ...
            if _ca.button("➕ 항목 추가", key="ei_add_err", use_container_width=True):
                if others and avail_turns:
                    ...
                    st.session_state.ei_counter += 1
                    st.rerun()
            _do_find_combo = _cb.button("🔍 가능한 조합 찾기", key="btn_find_combo", use_container_width=True)
        if _do_find_combo:
...
_schedule_display = display_df.copy()
if _exchange_limit_active:
    _ex_status = []
    for _nm in _schedule_display.index:
        _cnt = mgr.count_accepted_exchanges(_nm)
        _rem = max(0, _exchange_limit_count - _cnt)
        _ex_status.append('⛔' if _rem == 0 else f'🔄{_rem}')
    _schedule_display.insert(0, '교환', _ex_status)

st.dataframe(
    _schedule_display.style.apply(
        lambda row: style_table(row) if '교환' not in row.index
        else [''] + style_table(row.drop('교환')),
        axis=1),
    use_container_width=True, height=500
)
st.caption("🟪 연보라 음영 = 휴가 턴" + ("  |  🔄 = 교환 가능 횟수, ⛔ = 교환 불가" if _exchange_limit_active else ""))
...
with tab_browse:
    if not others_posts:
        ...
        _mkt_available = []
        _mkt_no_chance = []
        for _p in others_posts:
            _poster = _p.get('등록자', '')
            if _exchange_limit_active and mgr.count_accepted_exchanges(_poster) >= _exchange_limit_count:
                _mkt_no_chance.append(_p)
            else:
                _mkt_available.append(_p)

        if _mkt_no_chance:
            st.caption(f"활성 게시물 {len(others_posts)}건 (교환 기회 소진: {len(_mkt_no_chance)}건)")
        else:
            st.caption(f"활성 게시물 {len(others_posts)}건")

        # 기회 소진 게시글은 뒤에 배치
        _mkt_sorted = _mkt_available + _mkt_no_chance

        for post in _mkt_sorted:
            poster    = post.get('등록자', '')
            give_turn = post.get('주고싶은턴', '')
            ...
            # 게시자 교환 기회 체크
            _poster_no_chance = False
            if _exchange_limit_active:
                _poster_no_chance = mgr.count_accepted_exchanges(poster) >= _exchange_limit_count

            # 나의 교환 기회도 체크
            _my_no_chance = _exchange_limit_active and _my_exchange_count >= _exchange_limit_count

            compatibilities = mgr.get_market_compatibilities(user, post)
            valid_cnt = sum(1 for c in compatibilities if c['valid'])
            has_valid = valid_cnt > 0 and not _poster_no_chance and not _my_no_chance
            if give_turn == '아무턴':
                turn_label_mkt = '아무 턴'
            elif give_turn:
            ...
            _poster_tag = f" ⛔기회소진" if _poster_no_chance else ""
            _valid_label = f"✅ {valid_cnt}개 조합" if has_valid else ("⛔ 교환 불가" if _poster_no_chance else "❌ 불가")
            with st.expander(
                f"{'🟢' if has_valid else '🔴'} **{poster}**{_poster_tag} | "
                ...
                if not compatibilities:
                    ...
                    for combo in compatibilities:
                        ...
                        with col_b:
                            if _poster_no_chance or _my_no_chance:
                                st.caption("⛔")
                            elif is_valid and not has_pend:
...
> 모든 교환은 **필수과목(IM, GS, OB, PE, 진로탐색) 규칙**과 **휴가턴 규칙**을 자동으로 검증해서
...
| 🏖️ 휴가턴 교환 | 휴가턴을 줬으면 **같은 상대방에게서 다른 휴가턴을 받아와야** 해요 (예: 5턴 휴가를 주고, 9턴 휴가를 받기) |
